...
def learningRate(chapter_index):
    ...
    <body>
        <script>
            ...
            $(window.top.document).find(".stHidden").parent().hide();
            ...
            var sidebar_container = $(window.top.document).find("section[data-testid='stSidebar']").find("div[data-testid='stVerticalBlock']");
            ...
            if (learning_rate == 100) {
                $(window.top.document).find("#progress-text").text("已完成");
            } else {
                $(window.top.document).find("#progress-text").text(learning_rate + "%%");
            };
            //滚动条滚动事件
            ...
            //按钮绑定事件
            $(window.top.document).find("#clearLearningRate").click(function(){
                learning_rate = 0;
                $(section).scrollTop(0);
                learning_cookie["timestamp"] = parseInt(new Date().getTime() / 1000);
                learning_cookie[learning_mode][chapter] = learning_rate;
